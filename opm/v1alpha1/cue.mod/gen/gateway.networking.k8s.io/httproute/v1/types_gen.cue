...
#HTTPRouteSpec: {
	...
	rules?: list.MaxItems(16) & [...{
		...
		backendRefs?: list.MaxItems(16) & [...{
			...
			filters?: list.MaxItems(16) & [...{
				...
				// ExternalAuth configures settings related to sending request
				// details
				// to an external auth service. The external service MUST
				// authenticate
				// the request, and MAY authorize the request as well.
				//
				// If there is any problem communicating with the external
				// service,
				...
				externalAuth?: {
					...
					// requests to.
					//
					// The backend must speak the selected protocol (GRPC or HTTP) on
					// the
					// referenced port.
					//
					// If the backend service requires TLS, use BackendTLSPolicy to
					// tell the
					// implementation to supply the TLS details to be used to connect
					// to that
					// backend.
					backendRef!: {
						// Group is the group of the referent. For example,
					...
					forwardBody?: {
						// MaxSize specifies how large in bytes the largest body that will
						// be buffered
						// and sent to the authorization server. If the body size is
						// larger than
						// `maxSize`, then the body sent to the authorization server must
						// be
						...
						// See
						// https://github.com/kubernetes-sigs/gateway-api/pull/4001#discussion_r2291405746
						// for more.
						//
					...
					grpc?: {
						// AllowedRequestHeaders specifies what headers from the client
						// request
					...
					// HTTPAuthConfig contains configuration for communication with
					// HTTP-speaking
					// backends.
					//
					// If unset, implementations must assume the default behavior for
					// each
					// included field is intended.
					http?: {
						// AllowedRequestHeaders specifies what additional headers from
						// the client request
						// will be sent to the authorization server.
						//
						// The following headers must always be sent to the authorization
						// server,
						...
						//
						// Note that `Content-Length` has a special behavior, in that the
						// length
						// sent must be correct for the actual request to the external
						// authorization
						// server - that is, it must reflect the actual number of bytes
						// sent in the
						// body of the request to the authorization server.
						//
						// So if the `forwardBody` stanza is unset, or
						// `forwardBody.maxSize` is set
						// to `0`, then `Content-Length` must be `0`. If
						// `forwardBody.maxSize` is set
						// to anything other than `0`, then the `Content-Length` of the
						// authorization
						// request must be set to the actual number of bytes forwarded.
						allowedHeaders?: list.MaxItems(64) & [...string]

						// AllowedResponseHeaders specifies what headers from the
						// authorization response
						// will be copied into the request to the backend.
						//
						// If this list is empty, then all headers from the authorization
						// server
						// except Authority or Host must be copied.
						allowedResponseHeaders?: list.MaxItems(64) & [...string]

						// Path sets the prefix that paths from the client request will
						// have added
						// when forwarded to the authorization server.
						//
						// When empty or unspecified, no prefix is added.
						//
						// Valid values are the same as the "value" regex for path values
						// in the `match`
						// stanza, and the validation regex will screen out invalid paths
						// in the same way.
						// Even with the validation, implementations MUST sanitize this
						// input before using it
						// directly.
						path?: strings.MaxRunes(1024) & {
							=~"^(?:[-A-Za-z0-9/._~!$&'()*+,;=:@]|[%][0-9a-fA-F]{2})+$"
						}
					}

					// ExternalAuthProtocol describes which protocol to use when
					// communicating with an
					// ext_authz authorization server.
					//
					// When this is set to GRPC, each backend must use the Envoy
					// ext_authz protocol
					// on the port specified in `backendRefs`. Requests and responses
					// are defined
					// in the protobufs explained at:
					// https://www.envoyproxy.io/docs/envoy/latest/api-v3/service/auth/v3/external_auth.proto
					//
					// When this is set to HTTP, each backend must respond with a
					// `200` status
					// code in on a successful authorization. Any other code is
					// considered
				...
				requestHeaderModifier?: {
					...
					add?: list.MaxItems(16) & [...{
						...
						//
						// Must consist of printable US-ASCII characters, optionally
						// separated
						// by single tabs or spaces. See:
						// https://tools.ietf.org/html/rfc7230#section-3.2
		...
		// Support: Extended
		retry?: {
			// Attempts specifies the maximum number of times an individual
			// request
	...
	// UseDefaultGateways indicates the default Gateway scope to use
	// for this
	// Route. If unset (the default) or set to None, the Route will
	// not be
	// attached to any default Gateway; if set, it will be attached to
	// any
	// default Gateway supporting the named scope, subject to the
	// usual rules
	// about which Routes a Gateway is allowed to claim.
	//
	// Think carefully before using this functionality! The set of
	// default
	// Gateways supporting the requested scope can change over time
	// without
	// any notice to the Route author, and in many situations it will
	// not be
	// appropriate to request a default Gateway for a given Route --
	// for
	// example, a Route with specific security requirements should
	// almost
