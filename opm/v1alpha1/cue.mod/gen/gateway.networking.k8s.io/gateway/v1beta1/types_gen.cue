...
#GatewaySpec: {
	...
	// MUST
	// indicate this in an associated entry in
	...
	addresses?: list.MaxItems(16) & [...matchN(1, [{
		...
		// Type of the address.
		type?: strings.MaxRunes(253) & strings.MinRunes(1) & {
			=~"^Hostname|IPAddress|NamedAddress|[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*\\/[A-Za-z0-9\\/\\-._~%!$&'()*+,;=:]+$"
		}

		// When a value is unspecified, an implementation SHOULD
		// automatically
		...
		value?: strings.MaxRunes(253)
	}]
	allowedListeners?: {
		// Namespaces defines which namespaces ListenerSets can be
		// attached to this Gateway.
		// The default value is to allow no ListenerSets.
		namespaces?: {
			// From indicates where ListenerSets can attach to this Gateway.
			// Possible
			// values are:
			//
			...
			// * All: ListenerSets in all namespaces may be attached to this
			// Gateway.
			// * None: Only listeners defined in the Gateway's spec are
			// allowed
			//
			// The default value None
			from?: "All" | "Selector" | "Same" | "None"

			// Selector must be specified when From is set to "Selector". In
			// that case,
			// only ListenerSets in Namespaces matching this Selector will be
			// selected by this
	...
	// DefaultScope, when set, configures the Gateway as a default
	// Gateway,
	// meaning it will dynamically and implicitly have Routes (e.g.
	// HTTPRoute)
	// attached to it, according to the scope configured here.
	//
	// If unset (the default) or set to None, the Gateway will not act
	// as a
	// default Gateway; if set, the Gateway will claim any Route with
	// a
	// matching scope set in its UseDefaultGateway field, subject to
	// the usual
	// rules about which routes the Gateway can attach to.
	//
	// Think carefully before using this functionality! While the
	// normal rules
	// about which Route can apply are still enforced, it is simply
	// easier for
	// the wrong Route to be accidentally attached to this Gateway in
	// this
	// configuration. If the Gateway operator is not also the operator
	// in
	// control of the scope (e.g. namespace) with tight controls and
	// checks on
	// what kind of workloads and Routes get added in that scope, we
	// strongly
	// recommend not using this just because it seems convenient, and
	// instead
	// stick to direct Route attachment.
	defaultScope?: "All" | "None"
	...
	infrastructure?: {
		...
		//
		// Support: Extended
		annotations?: struct.MaxFields(8) & {
			{
	...
	// Listener fields.
	//
	// When multiple listeners have the same value for the Protocol
	// field, then
	// each of the Listeners with matching Protocol values MUST have
	// different
	// values for other fields.
	//
	// The set of fields that MUST be different for a Listener differs
	// per protocol.
	// The following rules define the rules for what fields MUST be
	// considered for
	// Listeners to be distinct with each protocol currently defined
	// in the
	// Gateway API spec.
	//
	// The set of listeners that all share a protocol value MUST have
	// _different_
	...
	// * **TCP, UDP**: Port
	//
	// One **very** important rule to call out involves what happens
	// when an
	// implementation:
	//
	// * Supports TCP protocol Listeners, as well as HTTP, HTTPS, or
	// TLS protocol
	// Listeners, and
	// * sees HTTP, HTTPS, or TLS protocols with the same `port` as
	// one with TCP
	// Protocol.
	//
	...
	//
	// If an implementation does not support TCP Protocol Listeners,
	// then the
	// previous rule does not apply, and the TCP Listeners SHOULD NOT
	// be
	// accepted.
	//
	// Note that the `tls` field is not used for determining if a
	// listener is distinct, because
	// Listeners that _only_ differ on TLS config will still conflict
	// in all cases.
	//
	// ### Listeners that are distinct only by Hostname
	//
	// When the Listeners are distinct based only on Hostname, inbound
	// request
	...
	// then those
	// Listeners are _Conflicted_, and the implementation MUST set the
	// "Conflicted"
	// condition in the Listener Status to "True".
	//
	// The words "indistinct" and "conflicted" are considered
	// equivalent for the
	...
	//
	// Specifically, an implementation MAY accept a partial Listener
	// set subject to
	// the following rules:
	//
	...
	// * At least one distinct Listener MUST be present, or else the
	// Gateway effectively
	// contains _no_ Listeners, and must be rejected from processing
	// as a whole.
	//
	...
	//
	// Note that, for all distinct Listeners, requests SHOULD match at
	// most one Listener.
	...
	// Listener).
	//
	// This concept is known as "Listener Isolation", and it is an
	// Extended feature
	// of Gateway API. Implementations that do not support Listener
	// Isolation MUST
	// clearly document this, and MUST NOT claim support for the
	// `GatewayHTTPListenerIsolation` feature.
	//
	// Implementations that _do_ support Listener Isolation SHOULD
	// claim support
	// for the Extended `GatewayHTTPListenerIsolation` feature and
	// pass the associated
	// conformance tests.
	//
	...
	listeners!: list.MaxItems(64) & [...{
		...
		// request.
		// * HTTPS: The Listener Hostname SHOULD match both the SNI and
		// Host header.
		// Note that this does not require the SNI and Host header to be
		// the same.
		// The semantics of this are described in more detail below.
		//
		// To ensure security, Section 11.1 of RFC-6066 emphasizes that
		// server
		// implementations that rely on SNI hostname matching MUST also
		// verify
		// hostnames within the application protocol.
		//
		// Section 9.1.2 of RFC-7540 provides a mechanism for servers to
		// reject the
		// reuse of a connection by responding with the HTTP 421
		// Misdirected Request
		// status code. This indicates that the origin server has rejected
		// the
		// request because it appears to have been misdirected.
		//
		// To detect misdirected requests, Gateways SHOULD match the
		// authority of
		// the requests with all the SNI hostname(s) configured across all
		// the
		// Gateway Listeners on the same port and protocol:
		//
		// * If another Listener has an exact match or more specific
		// wildcard entry,
		// the Gateway SHOULD return a 421.
		// * If the current Listener (selected by SNI matching during
		// ClientHello)
		// does not match the Host:
		// * If another Listener does match the Host, the Gateway SHOULD
		// return a
		// 421.
		// * If no other Listener matches the Host, the Gateway MUST
		// return a
		// 404.
		//
	...
	// TLS specifies frontend and backend tls configuration for entire
	// gateway.
	//
	// Support: Extended
	tls?: {
		backend?: {
			// ClientCertificateRef references an object that contains a
			// client certificate
			// and its associated private key. It can reference standard
			...
			// referenced resource
			// does not exist) or is misconfigured (e.g., a Secret does not
			// contain the keys
			// named `tls.crt` and `tls.key`). In this case, the
			// `ResolvedRefs` condition
			// on the Gateway MUST be set to False with the Reason
			// `InvalidClientCertificateRef`
			// and the Message of the Condition MUST indicate why the
			// reference is invalid.
			//
			// * It refers to a resource in another namespace UNLESS there is
			// a ReferenceGrant
			// in the target namespace that allows the certificate to be
			// attached.
			...
			//
			// Support: Core - Reference to a Kubernetes TLS Secret (with the
			// type `kubernetes.io/tls`).
			// Support: Implementation-specific - Other resource kinds or
			// Secrets with a
		...
		frontend?: {
			default!: {
				// Validation holds configuration information for validating the
				// frontend (client).
				// Setting this field will result in mutual authentication when
				...
				validation?: {
					...
					//
					// * It refers to a resource that cannot be resolved (e.g., the
					// referenced resource does not exist) or is misconfigured (e.g.,
					// a
					// ConfigMap does not contain a key named `ca.crt`). In this case,
					// the
					// Reason on all matching HTTPS listeners must be set to
					...
					// `InvalidCACertificateKind` and the Message of the Condition
					// must explain
					...
					// certificate to be attached. If a ReferenceGrant does not allow
					// this
					// reference, the `ResolvedRefs` on all matching HTTPS listeners
					// condition
					...
					// specific formats).
					// In such cases, an implementation-specific Reason and Message
					// MUST be set.
					//
					// In all cases, the implementation MUST ensure that the
					// `ResolvedRefs`
					// condition is set to `status: False` on all targeted listeners
					// (i.e.,
					// listeners serving HTTPS on a matching port). The condition MUST
					// include a Reason and Message that indicate the cause of the
					// error. If
					// ALL CACertificateRefs are invalid, the implementation MUST also
					// ensure
					// the `Accepted` condition on the listener is set to `status:
					// False`, with
					...
					// FrontendValidationMode defines the mode for validating the
					// client certificate.
					// There are two possible modes:
					//
					// - AllowValidOnly: In this mode, the gateway will accept
					// connections only if
					// the client presents a valid certificate. This certificate must
					// successfully
					// pass validation against the CA certificates specified in
					// `CACertificateRefs`.
					// - AllowInsecureFallback: In this mode, the gateway will accept
					// connections
					// even if the client certificate is not presented or fails
					// verification.
					//
					// This approach delegates client authorization to the backend and
					// introduce
					// a significant security risk. It should be used in testing
					// environments or
			...
			//
			// support: Core
			perPort?: list.MaxItems(64) & [...{
				// The Port indicates the Port Number to which the TLS
				// configuration will be
				// applied. This configuration will be applied to all Listeners
				// handling HTTPS
