...
import (
	"list"
	k8sbatchv1 "opmodel.dev/opm/schemas/kubernetes/batch/v1@v1"
	...
	security_traits "opmodel.dev/opm/traits/security@v1"
...
#JobTransformer: transformer.#Transformer & {
	...
	requiredResources: {
		"opmodel.dev/opm/resources/workload/container@v1": workload_resources.#ContainerResource
	}

	// Optional resources
	optionalResources: {
		"opmodel.dev/opm/resources/storage/volumes@v1": storage_resources.#VolumesResource
	}

	// Required traits - JobConfig is mandatory for Job
	requiredTraits: {
		"opmodel.dev/opm/traits/workload/job-config@v1": workload_traits.#JobConfigTrait
	}

	// Optional traits
	optionalTraits: {
		"opmodel.dev/opm/traits/workload/restart-policy@v1":     workload_traits.#RestartPolicyTrait
		"opmodel.dev/opm/traits/workload/sidecar-containers@v1": workload_traits.#SidecarContainersTrait
		"opmodel.dev/opm/traits/workload/init-containers@v1":    workload_traits.#InitContainersTrait
		"opmodel.dev/opm/traits/security/security-context@v1":   security_traits.#SecurityContextTrait
		"opmodel.dev/opm/traits/security/workload-identity@v1":  security_traits.#WorkloadIdentityTrait
	}
	...
	#transform: {
		...
		_sidecarContainers: *optionalTraits["opmodel.dev/opm/traits/workload/sidecar-containers@v1"].#defaults | [...]
		if #component.spec.sidecarContainers != _|_ {
			_sidecarContainers: #component.spec.sidecarContainers
		}

		_initContainers: *optionalTraits["opmodel.dev/opm/traits/workload/init-containers@v1"].#defaults | [...]
		...
		output: k8sbatchv1.#Job & {
			...
			spec: {
				completions: *requiredTraits["opmodel.dev/opm/traits/workload/job-config@v1"].#defaults.completions | int
				if _jobConfig.completions != _|_ {
					completions: _jobConfig.completions
				}

				parallelism: *requiredTraits["opmodel.dev/opm/traits/workload/job-config@v1"].#defaults.parallelism | int
				if _jobConfig.parallelism != _|_ {
					parallelism: _jobConfig.parallelism
				}

				backoffLimit: *requiredTraits["opmodel.dev/opm/traits/workload/job-config@v1"].#defaults.backoffLimit | int
				if _jobConfig.backoffLimit != _|_ {
					backoffLimit: _jobConfig.backoffLimit
				}

				activeDeadlineSeconds: *requiredTraits["opmodel.dev/opm/traits/workload/job-config@v1"].#defaults.activeDeadlineSeconds | int
				if _jobConfig.activeDeadlineSeconds != _|_ {
					activeDeadlineSeconds: _jobConfig.activeDeadlineSeconds
				}

				ttlSecondsAfterFinished: *requiredTraits["opmodel.dev/opm/traits/workload/job-config@v1"].#defaults.ttlSecondsAfterFinished | int
