...
- [ ] 2.2 Set `requiredResources` to `"opmodel.dev/resources/config@v0#ConfigMap": config_resources.#ConfigMapResource`, no requiredLabels/requiredTraits
- [ ] 2.3 Implement `#transform` to emit `v1/ConfigMap` — extract `#component.spec.configMap`, emit `metadata` from `#context`, `data` from resource spec
- [ ] 2.4 Add test component `_testConfigMapComponent` with ConfigMap resource to `test_data.cue`, add `_testConfigMapTransformer` validation
...
- [ ] 3.2 Set `requiredResources` to `"opmodel.dev/resources/config@v0#Secret": config_resources.#SecretResource`, no requiredLabels/requiredTraits
- [ ] 3.3 Implement `#transform` to emit `v1/Secret` — extract `#component.spec.secret`, emit with `type` (default `"Opaque"` from schema defaults) and `data`
- [ ] 3.4 Add test component `_testSecretComponent` and `_testSecretTransformer` to `test_data.cue`
...
- [ ] 4.2 Set `requiredResources` to `"opmodel.dev/resources/security@v0#WorkloadIdentity": security_resources.#WorkloadIdentityResource`
- [ ] 4.3 Implement `#transform` to emit `v1/ServiceAccount` — extract `#component.spec.workloadIdentity`, emit with `automountServiceAccountToken` from spec
...
- [ ] 4.5 Wire `serviceAccountName` into workload transformer pod spec when `#component.spec.workloadIdentity != _|_`
- [ ] 4.6 Add test component `_testServiceAccountComponent` and `_testServiceAccountTransformer` to `test_data.cue`
...
- [ ] 5.2 Set `requiredTraits` to `"opmodel.dev/traits/workload@v0#Scaling": workload_traits.#ScalingTrait`
- [ ] 5.3 Implement `#transform` to emit `autoscaling/v2/HorizontalPodAutoscaler` when `#component.spec.scaling.auto != _|_`; emit empty struct when `auto` is absent
...
- [ ] 5.5 Set `scaleTargetRef` by reading `#component.metadata.labels["core.opmodel.dev/workload-type"]` and mapping to K8s kind
- [ ] 5.6 Handle optional `behavior` field — conditionally include `spec.behavior` from `auto.behavior`
- [ ] 5.7 Update Deployment and StatefulSet transformers to use `scaling.auto.min` as `spec.replicas` when `#component.spec.scaling.auto != _|_`
- [ ] 5.8 Add test component `_testHPAComponent` (stateless with `scaling.auto` config) and `_testHPATransformer` to `test_data.cue`
...
- [ ] 6.2 Set `requiredTraits` to `"opmodel.dev/traits/network@v0#HttpRoute": network_traits.#HttpRouteTrait` only (no Expose, no Ingress trait)
- [ ] 6.3 Implement `#transform` to emit `networking.k8s.io/v1/Ingress` — extract `#component.spec.httpRoute`, backend service name from `#component.metadata.name`, backend port from `backendPort`
- [ ] 6.4 Map `httpRoute.hostnames` to Ingress `rules[].host` (omit `host` field if no hostnames), route `rules[].matches[].path` to Ingress paths
...
- [ ] 6.7 Default `pathType` to `"Prefix"` (already the schema default for `#HttpRouteMatchSchema.path.type`)
- [ ] 6.8 Add test component `_testIngressComponent` (with HttpRoute trait, hostnames, rules) and `_testIngressTransformer` to `test_data.cue`
...
- [ ] 7.1 Register all 5 new transformers in `providers/kubernetes/provider.cue` transformers map with FQN keys: `#ConfigMapTransformer`, `#SecretTransformer`, `#ServiceAccountTransformer`, `#HPATransformer`, `#IngressTransformer`
