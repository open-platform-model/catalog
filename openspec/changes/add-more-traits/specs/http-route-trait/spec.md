...
- `#RouteAttachmentSchema`: A struct with optional `gatewayRef` (struct with required `name` string and optional `namespace` string), optional `tls` (struct with optional `mode` defaulting to `"Terminate"` constrained to `"Terminate"` or `"Passthrough"`, and optional `certificateRef` struct with required `name` string and optional `namespace` string), and optional `className` (string).
...
- **THEN** the schema SHALL accept optional `gatewayRef`, `tls`, and `className` fields via `#RouteAttachmentSchema` embedding
...
- **WHEN** a component specifies `httpRoute: { className: "nginx", rules: [{ backendPort: 8080 }] }`
- **THEN** the schema SHALL accept the value

#### Scenario: Schema accepts all attachment fields together

- **WHEN** a component specifies `httpRoute: { className: "nginx", gatewayRef: { name: "gw" }, tls: { mode: "Terminate" }, hostnames: ["app.example.com"], rules: [{ backendPort: 8080 }] }`
- **THEN** the schema SHALL accept the value
