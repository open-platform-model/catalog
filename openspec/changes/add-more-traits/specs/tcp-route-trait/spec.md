...
The traits module SHALL define a `#TcpRouteTrait` in `traits/network/tcp_route.cue` that wraps `#TcpRouteSchema`. The trait SHALL declare `appliesTo: [workload_resources.#ContainerResource]`. The trait SHALL NOT provide defaults since TCP routing is application-specific.
