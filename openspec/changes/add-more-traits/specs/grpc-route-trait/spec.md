...
The traits module SHALL define a `#GrpcRouteTrait` in `traits/network/grpc_route.cue` that wraps `#GrpcRouteSchema`. The trait SHALL declare `appliesTo: [workload_resources.#ContainerResource]`. The trait SHALL NOT provide defaults since gRPC routing is application-specific.
