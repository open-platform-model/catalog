...
The traits module SHALL define a `#DisruptionBudgetTrait` in `traits/workload/disruption_budget.cue` that wraps `#DisruptionBudgetSchema`. The trait SHALL declare `appliesTo: [workload_resources.#ContainerResource]`. The trait SHALL NOT define defaults since disruption tolerance is workload-specific.
