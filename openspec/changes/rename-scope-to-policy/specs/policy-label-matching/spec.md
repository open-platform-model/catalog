...
The former `componentLabels` field on `#Scope.appliesTo` SHALL be removed. `matchLabels` replaces it with a clearer, flatter structure.
...
### Requirement: componentLabels field

**Reason**: Underspecified (`[string]: #LabelsAnnotationsType` — unclear what the outer key represents). Replaced by `matchLabels` with flat key-value equality matching.
**Migration**: Replace `componentLabels` with `matchLabels` using flat label key-value pairs.
