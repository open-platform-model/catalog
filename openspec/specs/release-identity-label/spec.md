...
Defines the `module-release.opmodel.dev/uuid` identity label on `#ModuleRelease.metadata.labels`, ensuring every module release carries a deterministic UUID that propagates through `#TransformerContext` to rendered provider resources.
