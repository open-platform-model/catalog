...
Specifies how the Kubernetes provider's workload transformers (Deployment, StatefulSet, DaemonSet, Job, CronJob) wire optional traits — HealthCheck, Sizing, and SecurityContext — into their K8s output. Covers probe emission, resource requests/limits, pod-level and container-level security context, and backward compatibility when traits are absent.
