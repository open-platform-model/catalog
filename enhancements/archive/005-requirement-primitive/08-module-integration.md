...
This document describes how module authors consume the backup primitives and how release authors configure them per environment. The examples use Approach C's trait syntax for clarity — the module author experience is similar across all approaches; the main difference is whether the hook is a separate trait or an inline field.

---

## Module-Side: Defining Backup Capability

A module author adds backup support by applying the primitives to the component that owns the data:
...
- The primitive is applied to the same component it protects — co-location makes the relationship explicit
