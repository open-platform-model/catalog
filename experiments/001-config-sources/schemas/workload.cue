...
// Environment variable entry — either a literal value or a reference to a config source.
// Exactly one of value or from MUST be set.
#EnvVarSchema: {
	name!: string

	// Literal value (backward compatible with existing usage)
	value?: string

	// Reference to a key in a named config source (intended for external resources)
	from?: {
		source!: string // Name of the config source
		key!:    string // Key within that source
	}

	// Enforce: exactly one of value or from must be set
	...
	_hasOneSource: _hasValue | _hasFrom
	_hasOneSource: true
	if _hasValue && _hasFrom {
		_conflict: _|_
	}
}
